...
class AliasOrderLog extends DataExtractor{
  override def getRDD(sc: SparkContext, argv: ExecutorArgs): RDD[Map[String, Any]] = {
    ...
    var querySql: String = s"""
         ...
         | from
         |     (
         |        select order_id, sku_id, dt
         |        from
         |            app.app_cps_union_new_ref_status_result
         |        where 
         |            dt = '${date}'
         |     )a
         |     join(
         ...
         |            dt = '${date}'
         |     )b
         |     on a.order_id = b.order_id and a.sku_id = b.sku_id and a.dt = b.dt
       """.stripMargin
