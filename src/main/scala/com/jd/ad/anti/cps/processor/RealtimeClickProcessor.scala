...
//      }
    ...
    // 获取实时点击流数据
    val clkStream = extractor.getDStream(ssc, conf, statistic)
    // Run Http Rule
...
//    httpFraudStream.print(2)
    ...
    // Run Dns Rule
    val dnsFraudStream = clkStream.window(Duration(conf.getWindowDuration()), Duration(conf.getWindowSlideDuration()))
                                  .transform((rdd: RDD[Map[String, Any]]) => {
